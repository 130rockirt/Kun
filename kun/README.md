...
- The GUI materializes the official read-only GitHub MCP server as a system-managed default. At connection time Kun uses `GITHUB_PAT_TOKEN`, then falls back to the credential from `gh auth token`; run `gh auth login` if neither is available. Kun materializes the token only in process memory—the persisted header keeps an environment reference and never contains the token itself. A user-defined `github` server in `~/.kun/mcp.json` takes precedence.
