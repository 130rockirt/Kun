...
- Use control edges for ordering and data edges for named result handoffs. A data-edge name labels the bounded result packet you will approve for the successor; it does not require the executor to publish an artifact. Avoid worker-to-worker message flow. Use explicit completion nodes and only bounded LoopGates. Ordinary dependencies must remain acyclic.
