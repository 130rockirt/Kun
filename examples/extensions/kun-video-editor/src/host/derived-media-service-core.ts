...
import type { DerivedMediaServiceOptions, DerivedMediaStartInput, PendingOutput, PendingStage } from './derived-media-service-model.js'
import {
 ...
 */
export class DerivedMediaServiceCore {
  protected storePromise?: Promise<DerivedMediaStore>
  protected projectService?: ProjectService

  constructor(
    protected readonly context: ExtensionContext,
    protected readonly options: DerivedMediaServiceOptions = {}
  ) {}


  protected async requestRecord(
  ...
  protected async reconcile(filter: { projectId: string }): Promise<void> {
    const store = await this.store()
  ...
  protected async verifiedArtifacts(
  ...
  protected async finalizeInvalidation(
    store: DerivedMediaStore,
    record: DerivedMediaRecord
  ): Promise<DerivedMediaRecord> {
    if (record.jobId) {
  ...
  protected async synchronizeRecordAccess(
    store: DerivedMediaStore,
    projectId: string
  ): Promise<void> {
  ...
  protected async loadProject(projectId: string): Promise<VideoProject | undefined> {
  ...
  protected assertOwnedSnapshot(snapshot: JobSnapshot): void {
    if (
  ...
  protected async scheduleQueued(projectId: string): Promise<void> {
    const store = await this.store()
    const records = await store.list({ owner: this.owner({ projectId }) })
  ...
  protected async scheduleRecord(
    record: DerivedMediaRecord,
  ...
  protected async hasExportWork(records: readonly DerivedMediaRecord[]): Promise<boolean> {
  ...
  protected async savePendingOutput(pending: PendingOutput): Promise<void> {
  ...
  protected async pendingOutput(recordId: string): Promise<PendingOutput | undefined> {
  ...
  protected async pendingHandleIds(recordId: string): Promise<string[]> {
    const value = await this.context.storage.workspace.get<JsonValue>(outputKey(recordId))
  ...
  protected async releaseRecordHandles(record: DerivedMediaRecord): Promise<void> {
    const pending = await this.pendingOutput(record.id)
  ...
  protected async discardPending(record: DerivedMediaRecord): Promise<void> {
    await this.releaseRecordHandles(record)
    await this.context.storage.workspace.delete(outputKey(record.id)).catch(() => false)
  }

  protected async releaseHandles(handles: readonly string[]): Promise<void> {
    await Promise.all([...new Set(handles)].map((handleId) =>
      this.context.media.release({ resource: 'handle', handleId }).catch(() => undefined)
    ))
  }

  protected async publish(record: DerivedMediaRecord, reason: string): Promise<void> {
  ...
  protected owner(input: { projectId?: string; assetId?: string } = {}): {
    extensionId: string
  ...
  protected async store(): Promise<DerivedMediaStore> {
